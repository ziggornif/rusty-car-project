...
fn main() -> ! {
    ...
    info!("ESP-NOW initialisé - Recherche du controleur...");

    let mut controller_address: Option<[u8; 6]> = None;
    ...
    loop {
        ...
        if let Some(r) = r {
            ...
            if dst != BROADCAST_ADDRESS && message_str == "CONNECTED" {
                info!("Controleur découvert !");
                controller_address = Some(src);
            }

            if dst == BROADCAST_ADDRESS && controller_address.is_none() {
                info!("Controleur découvert !");
                controller_address = Some(src);
        ...
        if controller_address.is_none() && time::Instant::now() >= next_discovery_broadcast {
            next_discovery_broadcast = time::Instant::now() + Duration::from_secs(2);
            let status = esp_now
                .send(&BROADCAST_ADDRESS, b"ROBOT_PING")
                .unwrap()
                .wait();
